...
class ClassAttendanceTest(TestCase):
    ...
    def test_with_geolocation_update(self):
        ...
        self.assertEqual(
            class_attendance.get_attendance_status(), AttendanceStatus.Present
        )
        ...
        for (
            ...
            geo_status,
            expected_result,
        ) in ClassAttendanceTest.test_comninations:
            ...
            result = ClassAttendance.objects.get(
                pk=class_attendance.pk
            ).get_attendance_status()
            self.assertEqual(
                expected_result,
                result,
                "With BSM status: {} & Geo status: {} Expected was: {} but found {}".format(  # noqa: E501
                    bsm_attendance.get_attendance_status(),
                    geo_attendance.get_attendance_status(),
                    expected_result,
                    result,
                ),
            )
